...
#define G 6.67e-11        // Constante de gravitación en N·m²/kg²
...
void inicializarCuerpos(Body cuerpos[]) {
    ...
    cuerpos[1].ry = 0.0;
    cuerpos[1].vx = 0.0;
    cuerpos[1].vy = sqrt(1.0 / fabs(cuerpos[1].rx)); // velocidad orbital para órbita circular
    cuerpos[1].m  = 3.0e-6;  // Masa reescalada de la Tierra (aprox. 3e-6 veces la masa solar)
}
